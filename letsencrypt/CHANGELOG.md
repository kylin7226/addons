...
- Add Alibaba Cloud (Aliyun) DNS provider support (`dns-aliyun`)
  - Support for automatic DNS-01 challenge via Aliyun DNS API
  - Ideal for users with domains resolved by Aliyun DNS
  - Supports wildcard certificate requests
  - Implemented as a legacy provider using `certbot-dns-aliyun` plugin
- Add Chinese translation (`zh.yaml`) for Home Assistant UI
- Update documentation to reflect custom repository features
